...
#define FT_DECLARE_DRIVER(class_) \
  FT_CALLBACK_TABLE const FT_Driver_ClassRec class_;

#define FT_DEFINE_DRIVER(class_, flags_, size_, name_, version_, requires_, \
  ...
  FT_CALLBACK_TABLE_DEF const FT_Driver_ClassRec class_ = {                 \
      FT_DEFINE_ROOT_MODULE(flags_, size_, name_, version_, requires_,      \
                            ...
                            get_interface_) face_object_size_,              \
      size_object_size_, slot_object_size_, init_face_, done_face_,         \
      init_size_, done_size_, init_slot_, done_slot_,                       \
      FT_DEFINE_DRIVERS_OLD_INTERNALS(old_set_char_sizes_,                  \
                                      old_set_pixel_sizes_) load_glyph_,    \
      get_kerning_, attach_file_, get_advances_, request_size_, select_size_};
...
#define FT_DEFINE_DRIVER(class_, flags_, size_, name_, version_, requires_,    \
  ...
  FT_Error FT_Create_Class_##class_(FT_Library library,                        \
    ...
    FT_DEFINE_ROOT_MODULE(                                                     \
        ...
        get_interface_) clazz->face_object_size = face_object_size_;           \
    ...
    clazz->done_slot = done_slot_;                                             \
                                                                               \
    FT_DEFINE_DRIVERS_OLD_INTERNALS(old_set_char_sizes_,                       \
                                    old_set_pixel_sizes_) clazz->load_glyph =  \
        load_glyph_;                                                           \
                                                                               \
