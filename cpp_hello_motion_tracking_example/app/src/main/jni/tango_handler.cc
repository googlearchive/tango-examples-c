...
namespace {
constexpr int kTangoCoreMinimumVersion = 6804;
...
void TangoHandler::OnResume() {
  ...
  if (TangoService_connect(nullptr, tango_config_) != TANGO_SUCCESS) {
    LOGE("TangoHandler::OnResume, TangoService_connect error.");
    std::exit(EXIT_SUCCESS);
  }
}
