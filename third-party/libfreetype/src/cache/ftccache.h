...
#ifndef FTC_INLINE
FT_LOCAL(FT_Error)
FTC_Cache_Lookup(FTC_Cache cache, FT_UInt32 hash, FT_Pointer query,
                 FTC_Node* anode);
#endif

FT_LOCAL(FT_Error)
FTC_Cache_NewNode(FTC_Cache cache, FT_UInt32 hash, FT_Pointer query,
                  FTC_Node* anode);
...
#define FTC_CACHE_LOOKUP_CMP(cache, nodecmp, hash, query, node, error) \
  FT_BEGIN_STMNT FTC_Node* _bucket, *_pnode, _node;                    \
  ...
  if (_idx < _cache->p) _idx = _hash & (_cache->mask * 2 + 1);         \
                                                                       \
  _bucket = _pnode = _cache->buckets + _idx;                           \
  for (;;) {                                                           \
    _node = *_pnode;                                                   \
    if (_node == NULL) goto _NewNode;                                  \
                                                                       \
    if (_node->hash == _hash && _nodcomp(_node, query, _cache)) break; \
                                                                       \
    _pnode = &_node->link;                                             \
  }                                                                    \
                                                                       \
...
#define FTC_CACHE_TRYLOOP_END()                                         \
  if (!error || error != FTC_Err_Out_Of_Memory) break;                  \
                                                                        \
