...
#define OTV_OPTIONAL_OFFSET(_offset) \
  FT_BEGIN_STMNT _offset##_p = p;    \
  _offset = FT_NEXT_USHORT(p);       \
  FT_END_STMNT

#define OTV_LIMIT_CHECK(_count)                                               \
  FT_BEGIN_STMNT if (p + (_count) > valid->root->limit) FT_INVALID_TOO_SHORT; \
  FT_END_STMNT

#define OTV_SIZE_CHECK(_size)                           \
  FT_BEGIN_STMNT if (_size > 0 && _size < table_size) { \
...
#define OTV_NEST1(x)                           \
  FT_BEGIN_STMNT valid->nesting_level = 0;     \
...
FT_LOCAL(void)
otv_Coverage_validate(FT_Bytes table, OTV_Validator valid,
