...
#define AF_ANGLE_DIFF(result, angle1, angle2)           \
  FT_BEGIN_STMNT AF_Angle _delta = (angle2) - (angle1); \
                                                        \
  _delta %= AF_ANGLE_2PI;                               \
  if (_delta < 0) _delta += AF_ANGLE_2PI;               \
                                                        \
  if (_delta > AF_ANGLE_PI) _delta -= AF_ANGLE_2PI;     \
                                                        \
  result = _delta;                                      \
  FT_END_STMNT
...
#define AF_SCALER_EQUAL_SCALES(a, b)                                           \
  ((a)->x_scale == (b)->x_scale&&(a)->y_scale == (b)->y_scale&&(a)->x_delta == \
   (b)->x_delta&&(a)->y_delta == (b)->y_delta)
...
#define AF_DEFINE_SCRIPT_CLASS(script_class, script_, ranges, m_size, m_init, \
                               m_scale, m_done, h_init, h_apply)              \
  FT_CALLBACK_TABLE_DEF const AF_ScriptClassRec script_class = {              \
      script_, ranges, m_size, m_init, m_scale, m_done, h_init, h_apply};

#else

#define AF_DECLARE_SCRIPT_CLASS(script_class) \
  FT_LOCAL(void) FT_Init_Class_##script_class(AF_ScriptClassRec* ac);

#define AF_DEFINE_SCRIPT_CLASS(script_class, script_, ranges, m_size, m_init, \
                               m_scale, m_done, h_init, h_apply)              \
  FT_LOCAL_DEF(void) FT_Init_Class_##script_class(AF_ScriptClassRec* ac) {    \
    ac->script = script_;                                                     \
