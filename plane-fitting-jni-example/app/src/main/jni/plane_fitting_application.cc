...
void PlaneFittingApplication::OnTouchEvent(float x, float y) {
  ...
  const glm::mat4 start_service_T_device = point_cloud_->GetCurrentTransform();
  ...
  if (TangoSupport_fitPlaneModelNearClick(
          current_cloud, color_camera_intrinsics_, glm::value_ptr(uv),
  ...
  const glm::mat4 opengl_world_T_depth_camera =
      opengl_world_T_start_service_ * start_service_T_device * device_T_depth_;

  // Transform to world coordinates
  const glm::vec4 world_position =
      opengl_world_T_depth_camera * glm::vec4(depth_position, 1.0f);

  glm::vec4 world_plane_equation;
  PlaneTransform(depth_plane_equation, opengl_world_T_depth_camera,
